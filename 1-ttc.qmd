...
- How to train a subword tokenizer (WordPiece, see @tip-tokenizer) on domain-specific text
...
To understand which words influenced a prediction, we use a technique called **integrated gradients** (via Captum's `LayerIntegratedGradients`). It assigns a score to each word in the input: a high score means that word pushed the model toward the predicted class.

::: {.callout-important}
## On the interpretation of integrated gradients
These scores are a useful guide, not a definitive explanation. Treat them as a way to explore the model's behavior, not as proof of causality.
:::
